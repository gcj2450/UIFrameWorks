...
namespace PathologicalGames
{
    ...
    ///         if (grid.UsePool && PathologicalGames.PoolManager.Pools.ContainsKey("GridItem"))
    ///      itemObject = PathologicalGames.PoolManager.Pools["GridItem"].Spawn(ItemPrefab.transform, Vector3.zero, Quaternion.identity, transform);
/// </description>
public static class PoolManager
    ...
    public class SpawnPoolsDict : IDictionary<string, SpawnPool>
    {
        ...
        internal bool Remove(SpawnPool spawnPool)
        {
            if (!this.ContainsKey(spawnPool.poolName))
            {
                UnityEngine.Debug.LogError(string.Format("PoolManager: Unable to remove '{0}'. " +
