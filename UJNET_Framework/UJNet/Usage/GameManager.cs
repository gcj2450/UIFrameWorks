...
public class GameManager : MonoBehaviour
{
	...
	public void OnResponse (int cmd, UJObject param)
	{
		switch (cmd) {
	...
	private void Load360GetOrder (UJObject param, int cmd)
	{
	...
	private void LoadUCGetOrder (UJObject param, int cmd)
	{
		if (param.GetInt ("state") == -1) {
    ...
    private void LoadDangLeGetOrder(UJObject param, int cmd)
    {
        if (param.GetInt("state") == -1)
        {
	...
	private void LoadMIGetOrder (UJObject param, int cmd)
	{
		if (param.GetInt ("state") == -1) {
	...
	private void LoadTStoreOrder (UJObject param, int cmd)
	{
	...
	private void LoadTStorePayCheck (UJObject param, int cmd)
	{
    ...
    //====================
    private void LoadCHINATELECOMOrder(UJObject param, int cmd)
    {
        //		Debug.LogWarning ("loadTStoreOrder : " + param.GetUtfString ("msg"));
        if (param.GetInt("state") == -1)
        {
    ...
    private void LoadCHINATELECOMPayCheck(UJObject param, int cmd)
    {
        //		Debug.LogWarning ("LoadTStorePayCheck : " + param.GetUtfString ("msg"));
        if (param.GetInt("state") == -1)
        {
	...
	//====================
	private void LoadOLLEHOrder (UJObject param, int cmd)
	{
		//		Debug.LogWarning ("loadTStoreOrder : " + param.GetUtfString ("msg"));
		if (param.GetInt ("state") == -1) {
	...
	private void LoadOLLEHPayCheck (UJObject param, int cmd)
	{
		//		Debug.LogWarning ("LoadTStorePayCheck : " + param.GetUtfString ("msg"));
		if (param.GetInt ("state") == -1) {
	...
	private void LoadLGUOrder (UJObject param, int cmd)
	{
		//		Debug.LogWarning ("loadTStoreOrder : " + param.GetUtfString ("msg"));
		if (param.GetInt ("state") == -1) {
	...
	private void LoadLGUPayCheck (UJObject param, int cmd)
	{
		//		Debug.LogWarning ("LoadTStorePayCheck : " + param.GetUtfString ("msg"));
		if (param.GetInt ("state") == -1) {
	...
	private void LoadWistoneGetOrder (UJObject param, int cmd)
	{
		if (param.GetInt ("state") == -1) {
	...
	private void LoadPaymentRequest (UJObject param, int cmd)
	{
		if (param.GetInt ("state") == -1) {
	...
	private void LoadPayCompleteList (UJObject param, int cmd)
	{
		...
		UJArray arr = param.GetUJArray ("data");
		if (arr != null) {
			//for (int i = 0; i < arr.Size(); i++) {
			//	RecoveryItemData itemData = new RecoveryItemData ();
			//	itemData.LoadFromUJObject (arr.GetUJObject (i));
