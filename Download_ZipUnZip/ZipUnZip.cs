...
public class ZipUnZip : MonoBehaviour
{
    private void Update()
    {
        if (Input.GetKeyUp(KeyCode.A))
        {
            ZipDirectory("D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets/base2", "D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets", "show");
        }
        if (Input.GetKeyUp(KeyCode.B))
        {
            UnCompressAsync("D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets/show.zip", "D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets/baseUnzip", showProgress);
            //UnZip("D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets/show.zip","D:/SourceTree/AssetBundleDemo/demo/Assets/StreamingAssets/baseUnzip");
        }
    }

    private void showProgress(string name, SharpZipResult result)
    {
        if (result.zs==ZipStatus.ZS_OK)
        {
            Debug.Log("UnZip OK");
        }
        else if (result.zs==ZipStatus.ZS_PROGRESS)
        {
            Debug.Log("UnZip : " + result.name + " progres: " + result.progress);
        }
        else if (result.zs == ZipStatus.ZS_ERROR)
        {
            Debug.Log("Unzip file  error = " + result.message);
        }
    }
    ...
    /// <param name="uncompressPath"></param>
    /// <param name="handler"></param>
    public void UnCompressAsync(string filePath, string uncompressPath, UnZipCompletedHandler handler)
    {
        SharpZipToolkit.UnZipToFolderAsync(filePath, uncompressPath, handler);
    }
    ...
    public static void ZipFile(string FileToZip, string ZipedPath, string ZipedFileName = "", int CompressionLevel = 9, int BlockSize = 2048)
    {
        //如果文件没有找到，则报错
